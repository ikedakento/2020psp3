...
## ソースコードの説明
l79 新しいNodeのメモリを確保する
l80~l83 確保したメモリを挿入する
l92~l93 要素を先頭に追加して前の要素とする
l94すべての要素にループを回す
l97消去する要素のメモリを解放
最後に到達したらERRORで返す

l124　不一致の時-1を出力させる 
l125~128 メモリを確保して挿入する
...
## 出力結果
リストの先頭に新しいデータを挿入した後の新しいアドレス

IDが合致した場所、IDが一致しなかった場合は-1
IDが合致した市町村のデータ

チャレンジ１
リストから消去に成功したらSUCCESS 失敗したらERROR
```
